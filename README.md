...
The final console message shows only the successful conversion count and says "See log file for additional details." Individual file, folder-access, and cleanup errors are recorded only in the log, so inaccessible folders do not flood the console. Fatal errors that stop the scan still appear in the console.

Exit code `0` means the scan completed without reported errors; existing-PDF and directory-link skips do not count as errors. Exit code `2` means the scan completed with individual conversion or scan/cleanup issues. Exit code `1` means a fatal error stopped the run. The launcher preserves this exit code. If the log cannot be created, the script reports the error in the console and stops before converting files.
